...
function getOrCreatePlayer(name) {
  ...
  if (!player) {
    const insert = db.prepare(`
      INSERT INTO players (name, chips, balance, total_games, total_wins, total_losses, total_pushes, biggest_win)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    `);
    insert.run(name, 0, 0, 0, 0, 0, 0, 0);
...
function updatePlayerStats(name, result, payout) {
  ...
  let newTotalWins = player.total_wins;
  let newTotalLosses = player.total_losses;
  let newTotalPushes = player.total_pushes;
  let newBiggestWin = player.biggest_win;
